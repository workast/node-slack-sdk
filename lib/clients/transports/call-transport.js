...
var handleRateLimitResponse = function handleRateLimitResponse(retryArgs, headers) {
  ...
  if (retryArgs.retryOp._options.noRetry) {
    /* eslint max-len: 0 */
    client.logger('info', 'Rate limited (' + get(retryArgs.task.args, ['url']) + '), disabled retry');
    return true;
  }
...
var handleTransportResponse = function handleTransportResponse(
  ...
  retryArgs.client.logger('verbose', 'StatusCode: '
    + statusCode +
    ', Task ' + JSON.stringify(retryArgs.task));
